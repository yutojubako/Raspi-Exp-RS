...
void bsp_InitADS1256(void)
{
	...
	//ADS1256_CfgADC(ADS1256_GAIN_1, ADS1256_1000SPS);	/* 配置ADC参数： 增襾E:1, 数据输出速率 1KHz */
}
...
int  main()
{
   ...
   // ADS1256_CfgADC(ADS1256_GAIN_1, ADS1256_15SPS);
	...
	while (1)
	{
		...
		for (i = 0; i < ch_num; i++)
		{
			sprintf(sendBuffer + 8*i, "%8ld", adc[i]);
		}
		udp_send(server_sock, sendBuffer, strlen(sendBuffer), &servSockAddr);
		//counter++;
		//printf("counter:%d\n", counter);
		//bsp_DelayUS(100000);
	}
